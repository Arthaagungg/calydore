...
if ($tab === 'villa') {
    ...
    if (!isset($_SESSION['shuffled_villa_kamar_ids'])) {
        echo json_encode([
            'status' => 'error',
    ...
    $shuffledIds = $_SESSION['shuffled_villa_kamar_ids'];
    ...
    $_SESSION['shuffled_villa_kamar_ids'] = array_slice($_SESSION['shuffled_villa_kamar_ids'], $limit);    // Query untuk mengambil data villa berdasarkan ID yang sudah diacak
    if (!empty($idsToFetch)) {
        ...
        $query = "SELECT * FROM villa_kamar WHERE 1 = 0"; // Tidak ada data
    }
} else {
...
$totalVillaQuery = "SELECT COUNT(*) AS total FROM villa_kamar";
