...
if ($tab === 'villa') {
    ...
    if (!isset($_SESSION['shuffled_glamping_ids'])) {
        echo json_encode([
            'status' => 'error',
    ...
    $shuffledIds = $_SESSION['shuffled_glamping_ids'];
    ...
    $_SESSION['shuffled_glamping_ids'] = array_slice($_SESSION['shuffled_glamping_ids'], $limit);    // Query untuk mengambil data villa berdasarkan ID yang sudah diacak
    if (!empty($idsToFetch)) {
        ...
        $query = "SELECT * FROM glamping WHERE 1 = 0"; // Tidak ada data
    }
} else {
...
$totalVillaQuery = "SELECT COUNT(*) AS total FROM glamping";
