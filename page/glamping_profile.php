...
if ($result->num_rows > 0) {
    ...
    echo '<script>
    alert("glamping Tidak Ditemukan!");
    window.location.href = "glamping.php";
    </script>';
    exit;
}
...
$pageTitle = 'Calydore | Glamping ' . $villa['nama_glamping'] . ' Dengan View Alam Di Cisarua Puncak Bogor';
$description = htmlspecialchars($villa['deskripsi'] ?? 'Rasakan pengalaman glamping unik di Puncak Cisarua, Bogor, dengan fasilitas mewah di tengah alam terbuka. Sewa tenda glamping yang nyaman untuk liburan penuh petualangan dan ketenangan. Nikmati suasana alam dengan kenyamanan maksimal!', ENT_QUOTES, 'UTF-8');
...
<div class="profile-container">
    ...
    <div class="profile-image">
        ...
        <div class="profile-detail">
            <div class="villa-details">
                <h2>Glamping <?php echo htmlspecialchars($villa['nama_glamping']); ?></h2>
    ...
    <div class="profile-rekomendasi">
        <h3>Glamping Rekomendasi</h3>
        <ul>
            <?php
            $query_recommendations = "
            SELECT v.id, v.nama_glamping, v.lokasi, v.rating, v.slug_glamping,
                (SELECT CONCAT(vi.file_path, vi.file_name) 
                    FROM glamping_images vi 
                    WHERE vi.glamping_id = v.id 
                    LIMIT 1) AS first_image
            FROM glamping v
            WHERE v.id <> " . intval($glamping['id']) . "
            ...
            if ($result_recommendations->num_rows > 0) {
                while ($recommended_villa = $result_recommendations->fetch_assoc()) {
                    ...
                    echo '
                        <li>
                            <a href="' . BASE_URL . '/glamping/' . htmlspecialchars($recommended_villa['slug_glamping']) . '">
                                <img src="' . htmlspecialchars($imageSrc) . '?tr=w-640,h-360,q-80" alt="Villa Image" class="recommended-image" loading="lazy">
                                <div class="recommended-details">
                                    <h4>Glamping ' . htmlspecialchars($recommended_villa['nama_glamping']) . '</h4>
...
<script type="application/ld+json">
{
  ...
  "@type": "Resort",
  "name": "Glamping <?php echo htmlspecialchars($villa['nama_glamping']); ?>",
  "url": "https://calydore.com/page/glamping/<?php echo urlencode($villa['slug_glamping']); ?>",
...
<?php
$ChatWa = "Permisi Kak? untuk Glamping " . htmlspecialchars($villa['nama_glamping']) . ", tanggal ... Ready ga ?";
