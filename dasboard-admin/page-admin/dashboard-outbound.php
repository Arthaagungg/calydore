            ...
            <main class="p-3">
                <div class="container-fluid">
                    <div class="mb-3 text-start">
                        <h3>Dashboard outbound</h3>
                        <button class="btn btn-primary" type="button" data-bs-toggle="modal"
                            data-bs-target="#Modaloutbound">Tambahkan outbound <i class="uil uil-plus"></i></button>
                    </div>
                    <!-- Modal -->
                    <div class="modal fade" id="Modaloutbound" tabindex="-1" aria-labelledby="exampleModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Tambahkan outbound</h5>
                                </div>
                                <div class="modal-body">
                                    <form action="../proses/upload-data.php" method="POST"
                                        ...
                                        <div class="form-section">
                                            <label for="category_outbound" class="form-label">Category Outbound </label>
                                            <input type="text" class="form-control" id="category_outbound"
                                                name="categorys[category_outbound]"
                                                placeholder="Contoh: game, rafting, paintball, offroud" required>
                                        </div>
                                        <div class="form-section">
                                            <label for="nama_outbound" class="form-label">Nama outbound</label>
                                            <input type="text" class="form-control" id="nama_outbound"
                                                name="categorys[nama_outbound]" required>
                                        </div>
                                        ...
                                        foreach ($categorys as $category) {
                                            ...
                                            if ($category == 'category_outbound') {
                                                $label = 'Kategori outbound (game/adventure)';
                                            }
                ...
                <!-- Menampilkan Semua outbound -->
                <div class="row d-flex flex-wrap justify-content-center">
                    <?php
                    // Mengecek jika data outbound ada
                    if ($result->num_rows > 0) {
                        // Loop untuk menampilkan setiap outbound
                        while ($outbound = $result->fetch_assoc()) {
                            // Query untuk mengambil gambar-gambar terkait outbound dari tabel outbound_images
                            $outbound_id = $outbound['id'];
                            $sql_images = "SELECT file_path, file_name FROM outbound_images WHERE outbound_id = ? LIMIT 1"; // Mengambil 1 gambar pertama
                            $stmt_images = $conn->prepare($sql_images);
                            $stmt_images->bind_param("i", $outbound_id);
                            $stmt_images->execute();
                            ...
                            <div class="col-md-4 mb-4">
                                <div class="card shadow-sm h-100">
                                    <img src="<?php echo htmlspecialchars($first_image) . "?tr=w-640,h-360,q-80"; ?>"
                                        class="card-img-top" alt="<?php echo htmlspecialchars($villa['nama_outbound']); ?>">
                                    <div class="card-body">
                                        <h5 class="card-title"><?php echo htmlspecialchars($outbound['nama_outbound']); ?></h5>
                                        <p class="card-text">
                                            <strong>Lokasi: </strong> <?php echo htmlspecialchars($outbound['lokasi']); ?><br>
                                            <strong>Kapasitas: </strong> <?php echo htmlspecialchars($outbound['kapasitas']); ?>
                                            Orang<br>
                                        </p>
                                        <!-- Form Edit outbound -->
                                        <div class="d-flex justify-content-between">
                                            <form action="../proses/edit-data.php" method="post" style="display:inline;">
                                                <input type="hidden" name="id" value="<?php echo $outbound['id']; ?>">
                                                <input type="hidden" name="category" value="outbound">
                                                <input type="hidden" name="csrf_token"
                                                    value="<?php echo $_SESSION['csrf_token']; ?>">
                                            ...
                                            <form action="../proses/delete-data.php" method="post" style="display:inline;">
                                                ...
                                                <button type="submit" class="btn btn-danger btn-sm"
                                                    onclick="return confirm('Apakah Anda yakin ingin menghapus outbound ini?')">Delete</button>
                                            </form>
                                        </div>
                        ...
                        echo '<p class="text-center">Tidak ada data outbound yang tersedia.</p>';
                    }
                    ?>
                </div>
